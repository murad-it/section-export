...
function importSections(array $sections): array
{
    $map = [];
    $pending = $sections;
    ...
    while (!empty($pending)) {
        ...
        foreach ($pending as $section) {
            ...
            $xmlId = $section['xml_id'];
            $parentXmlId = $section['parent_xml_id'];

            $parentId = null;

            if ($parentXmlId) {

                if (!isset($map[$parentXmlId])) {
                    $nextIteration[] = $section;
                    continue;
                }

                $parentId = $map[$parentXmlId];
            }
